                                      ...
                                      ProjectionConfiguration[] projectionConfiguration) 
{
    ...
    var valueFromIfrsVariable = ifrsVariables.Any() ? ifrsVariables.Max(ifrsVariable => ifrsVariable.Values.Length) : 1;
    ...
    int valueFromRawVariable = projectionConfiguration.Where(projConfig => projConfig.Shift < rawVariableMaxLength).Count();
...
public static class ComputationHelper{
    ...
    public static HashSet<DataType> CurrentPeriodCalculatedDataTypes = new []{DataType.Calculated, DataType.CalculatedTelescopic}.ToHashSet();
} 
