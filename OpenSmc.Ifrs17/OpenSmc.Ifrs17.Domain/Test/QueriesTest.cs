...
public bool Equal(double? a, double? b) => (a == null && b == null) || (a != null && b != null && Math.Abs((double)a - (double)b) < 1e-8);
                                           ...
                                           double? expectedLockedFirstYcValue = null)
{
    ...
    if (eurCurrentYieldCurve[gic] == null)
        return Activity.Finish();
    double? currentPrevPeriod = eurCurrentYieldCurve.TryGetValue(gic, out var ycByPeriod) && ycByPeriod != null && ycByPeriod.TryGetValue(PreviousPeriod, out var ycP) ? 
                                ycP?.Values.FirstOrDefault() : null;
    ...
    double? currentCurrPeriod = eurCurrentYieldCurve.TryGetValue(gic, out ycByPeriod) && ycByPeriod != null && ycByPeriod.TryGetValue(CurrentPeriod, out var ycC) ?
                                ycC?.Values.FirstOrDefault() : null;
