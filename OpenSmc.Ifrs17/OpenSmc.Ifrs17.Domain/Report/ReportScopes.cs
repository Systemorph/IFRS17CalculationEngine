...
public interface DataScope : IScope<(ReportIdentity ReportIdentity, string EstimateType), ReportStorage>, IDataCube<ReportVariable>
{
    ...
    {
        return builder.ForScope<DataScope>(s => s.WithApplicability<DataWrittenActual>(x => x.GetStorage().EstimateTypesWithoutAoc.Contains(x.Identity.EstimateType)));
    }
...
public interface FxData : IScope<(ReportIdentity ReportIdentity, CurrencyType CurrencyType, string EstimateType), ReportStorage>, IDataCube<ReportVariable>
{
    static ApplicabilityBuilder ScopeApplicabilityBuilder(ApplicabilityBuilder builder)
    {
        return builder.ForScope<FxData>(s => s.WithApplicability<FxDataWrittenActual>(x => x.GetStorage().EstimateTypesWithoutAoc.Contains(x.Identity.EstimateType)));
    }
...
public interface LrcActuarial : IScope<(ReportIdentity Id, CurrencyType CurrencyType), ReportStorage>, IDataCube<ReportVariable>
{
    static ApplicabilityBuilder ScopeApplicabilityBuilder(ApplicabilityBuilder builder)
    {
        return builder.ForScope<LrcActuarial>(s => s.WithApplicability<LrcActuarialPaa>(x => x.Identity.Id.ValuationApproach == ValuationApproaches.PAA));
    }
    ...
    IDataCube<ReportVariable> LrcActuarial.LrcActuarial => -1d * GetScope<Revenues>(Identity).Revenues + -1d * GetScope<Deferrals>(Identity).Deferrals + Loreco
                                                           + GetScope<BestEstimate>(Identity).BestEstimate
                                                               .Where(x => GetStorage().GetHierarchy<AmountType>().Ancestors(x.AmountType, true).Any(x => x.SystemName == AmountTypes.PR) ||
                                                                           GetStorage().GetHierarchy<AmountType>().Ancestors(x.AmountType, true).Any(x => x.SystemName == AmountTypes.DE))
                                                               .ToDataCube();
}
...
public interface ExperienceAdjustmentOnPremium : IScope<(ReportIdentity Id, CurrencyType CurrencyType), ReportStorage>, IDataCube<ReportVariable>
{
    static ApplicabilityBuilder ScopeApplicabilityBuilder(ApplicabilityBuilder builder)
    {
        return builder.ForScope<ExperienceAdjustmentOnPremium>(s => s.WithApplicability<ExperienceAdjustmentOnPremiumNotApplicable>(x => x.Identity.Id.IsReinsurance || x.Identity.Id.LiabilityType == LiabilityTypes.LIC));
    }
...
public interface InsuranceRevenue : IScope<(ReportIdentity Id, CurrencyType CurrencyType), ReportStorage>, IDataCube<ReportVariable>
{
    static ApplicabilityBuilder ScopeApplicabilityBuilder(ApplicabilityBuilder builder)
    {
        return builder.ForScope<InsuranceRevenue>(s => s.WithApplicability<InsuranceRevenueNotApplicable>(x => x.Identity.Id.IsReinsurance || x.Identity.Id.LiabilityType == LiabilityTypes.LIC));
    }
    ...
    private IDataCube<ReportVariable> ExpectedClaimsInvestmentComponent => -1 * CfOut
        .Where(x => GetStorage().GetHierarchy<AmountType>().Ancestors(x.AmountType, true).Any(x => x.SystemName == AmountTypes.ICO))
...
public interface InsuranceServiceExpense : IScope<(ReportIdentity Id, CurrencyType CurrencyType), ReportStorage>, IDataCube<ReportVariable>
{
    static ApplicabilityBuilder ScopeApplicabilityBuilder(ApplicabilityBuilder builder)
    {
        return builder.ForScope<InsuranceServiceExpense>(s => s.WithApplicability<InsuranceServiceExpenseReinsurance>(x => x.Identity.Id.IsReinsurance));
    }
